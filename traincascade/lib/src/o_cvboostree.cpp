...
CvBoostTree::CvBoostTree() { ensemble = nullptr; }

CvBoostTree::~CvBoostTree() { clear(); }

void CvBoostTree::clear() {
  CvDTree::clear();
  ensemble = nullptr;
}
...
bool CvBoostTree::train(const CvMat*, int, const CvMat*, const CvMat*,
...
bool CvBoostTree::train(CvDTreeTrainData*, const CvMat*) {
  assert(0);
  return false;
}

void CvBoostTree::scale(double _scale) {
...
void CvBoostTree::try_split_node(CvDTreeNode* node) {
  ...
  if (!node->left) {
    ...
    int i = 0, count = node->sample_count;
    double value = node->value;

    for (i = 0; i < count; i++) weak_eval[labels[i]] = value;
  }
}

double CvBoostTree::calc_node_dir(CvDTreeNode* node) {
  ...
  if (data->get_var_type(vi) >= 0)  // split on categorical var
  {
    ...
    for (i = 0; i < n; i++) {
      int idx =
          ((cat_labels[i] == 65535) && data->is_buf_16u) ? -1 : cat_labels[i];
      double w = weights[i];
  ...
  } else  // split on ordered var
  {
    ...
    for (; i < n; i++) dir[sorted_indices[i]] = (char)0;
  }
...
CvDTreeSplit* CvBoostTree::find_split_ord_class(CvDTreeNode* node, int vi,
                                                float init_quality,
                                                CvDTreeSplit* _split,
                                                uchar* _ext_buf) {
  ...
  if (!_ext_buf) inn_buf.allocate(n * (3 * sizeof(int) + sizeof(float)));
  ...
  if (split_criteria != CvBoost::GINI && split_criteria != CvBoost::MISCLASS)
    split_criteria =
        boost_type == CvBoost::DISCRETE ? CvBoost::MISCLASS : CvBoost::GINI;
...
CvDTreeSplit* CvBoostTree::find_split_cat_class(CvDTreeNode* node, int vi,
                                                float init_quality,
                                                CvDTreeSplit* _split,
                                                uchar* _ext_buf) {
  ...
  cv::AutoBuffer<uchar> inn_buf((2 * mi + 3) * sizeof(double) +
                                mi * sizeof(double*));
  ...
  // = k.
  for (j = -1; j < mi; j++) cjk[j * 2] = cjk[j * 2 + 1] = 0;
...
CvDTreeSplit* CvBoostTree::find_split_ord_reg(CvDTreeNode* node, int vi,
                                              float init_quality,
                                              CvDTreeSplit* _split,
                                              uchar* _ext_buf) {
  const float epsilon = FLT_EPSILON * 2;
  ...
  data->get_ord_var_data(node, vi, values_buf, indices_buf, &values, &indices,
                         sample_indices_buf);
  auto* responses_buf = (float*)(indices_buf + n);
...
CvDTreeSplit* CvBoostTree::find_split_cat_reg(CvDTreeNode* node, int vi,
                                              float init_quality,
                                              CvDTreeSplit* _split,
                                              uchar* _ext_buf) {
  const double* weights = ensemble->get_subtree_weights()->data.db;
  ...
  int* cat_labels_buf = (int*)ext_buf;
  const int* cat_labels = data->get_cat_var_data(node, vi, cat_labels_buf);
  auto* responses_buf = (float*)(cat_labels_buf + n);
  ...
  int i = 0, best_subset = -1, subset_i = 0;
...
CvDTreeSplit* CvBoostTree::find_surrogate_split_ord(CvDTreeNode* node, int vi,
                                                    uchar* _ext_buf) {
  const float epsilon = FLT_EPSILON * 2;
  int n = node->sample_count;
  cv::AutoBuffer<uchar> inn_buf;
  if (!_ext_buf) inn_buf.allocate(n * (2 * sizeof(int) + sizeof(float)));
  uchar* ext_buf = _ext_buf ? _ext_buf : inn_buf.data();
  auto* values_buf = (float*)ext_buf;
  int* indices_buf = (int*)(values_buf + n);
  int* sample_indices_buf = indices_buf + n;
...
CvDTreeSplit* CvBoostTree::find_surrogate_split_cat(CvDTreeNode* node, int vi,
                                                    uchar* _ext_buf) {
  const char* dir = (char*)data->direction->data.ptr;
  const double* weights = ensemble->get_subtree_weights()->data.db;
  int n = node->sample_count;
  int i = 0, mi = data->cat_count->data.i[data->get_var_type(vi)];

  int base_size = (2 * mi + 3) * sizeof(double);
  cv::AutoBuffer<uchar> inn_buf(base_size);
  if (!_ext_buf) inn_buf.allocate(base_size + n * sizeof(int));
  uchar* ext_buf = _ext_buf ? _ext_buf : inn_buf.data();
  int* cat_labels_buf = (int*)ext_buf;
  const int* cat_labels = data->get_cat_var_data(node, vi, cat_labels_buf);
...
void CvBoostTree::calc_node_value(CvDTreeNode* node) {
  ...
  if (data->is_classifier) {
    ...
    for (int k = 0; k < m; k++) cls_count[k] = 0;
    ...
    if (boost_type == CvBoost::DISCRETE) {
      ...
      // as the whole ensemble response is computes as
      // sign(sum_i(weak_response_i)
    ...
    double sum = 0, sum2 = 0, iw = NAN;
    auto* values_buf = (float*)(labels_buf + n);
