  ...
  if (isMakeRootCopy) {
    ...
    *root = *data_root;
    ...
    if (is_buf_16u) {
      ...
      for (int i = 0; i < count; i++) idst[i] = src_lbls[sidx[i]];
    }
    ...
    for (int vi = 0; vi < var_count; vi++) root->set_num_valid(vi, count);
  }
...
CvCascadeBoostTrainData::CvCascadeBoostTrainData(
  ...
  node_heap =
      cvCreateSet(0, sizeof(node_heap[0]), sizeof(CvDTreeNode), tree_storage);
  split_heap =
      cvCreateSet(0, sizeof(split_heap[0]), maxSplitSize, tree_storage);
}
...
CvCascadeBoostTrainData::CvCascadeBoostTrainData(
  ...
  setData(_featureEvaluator, _numSamples, _precalcValBufSize,
          _precalcIdxBufSize, _params);
}

void CvCascadeBoostTrainData::setData(
    const CvFeatureEvaluator* _featureEvaluator, int _numSamples,
    int _precalcValBufSize, int _precalcIdxBufSize,
    const CvDTreeParams& _params) {
  int* idst = nullptr;
  ...
  if (_precalcValBufSize < 0 || _precalcIdxBufSize < 0)
    CV_Error(CV_StsOutOfRange,
             "_numPrecalcVal and _numPrecalcIdx must be positive or 0");

  var_count = var_all =
      featureEvaluator->getNumFeatures() * featureEvaluator->getFeatureSize();
  sample_count = _numSamples;

  is_buf_16u = false;
  if (sample_count < 65536) is_buf_16u = true;

  numPrecalcVal = std::min(cvRound((double)_precalcValBufSize * 1048576. /
                                   (sizeof(float) * sample_count)),
                           var_count);
  numPrecalcIdx =
      std::min(cvRound((double)_precalcIdxBufSize * 1048576. /
                       ((is_buf_16u ? sizeof(unsigned short) : sizeof(int)) *
                        sample_count)),
               var_count);
  ...
  tree_storage = cvCreateMemStorage(treeBlockSize);
  node_heap =
      cvCreateSet(0, sizeof(*node_heap), sizeof(CvDTreeNode), tree_storage);
...
const int* CvCascadeBoostTrainData::get_class_labels(CvDTreeNode* n,
                                                     int* labelsBuf) {
...
const int* CvCascadeBoostTrainData::get_sample_indices(CvDTreeNode* n,
                                                       int* indicesBuf) {
  return CvDTreeTrainData::get_cat_var_data(n, get_work_var_count(),
                                            indicesBuf);
}

const int* CvCascadeBoostTrainData::get_cv_labels(CvDTreeNode* n,
                                                  int* labels_buf) {
  return CvDTreeTrainData::get_cat_var_data(n, get_work_var_count() - 1,
                                            labels_buf);
}
...
void CvCascadeBoostTrainData::get_ord_var_data(
  ...
  } else  // vi >= numPrecalcIdx
  {
    ...
    std::sort(sortedIndicesBuf, sortedIndicesBuf + nodeSampleCount,
              LessThanIdx<float, int>(&sampleValues[0]));
    for (int i = 0; i < nodeSampleCount; i++)
...
const int* CvCascadeBoostTrainData::get_cat_var_data(CvDTreeNode* n, int vi,
                                                     int* catValuesBuf) {
  int nodeSampleCount = n->sample_count;
...
struct FeatureIdxOnlyPrecalc : cv::ParallelLoopBody {
  FeatureIdxOnlyPrecalc(const CvFeatureEvaluator* _featureEvaluator,
                        CvMat* _buf, int _sample_count, bool _is_buf_16u) {
    featureEvaluator = _featureEvaluator;
  ...
  void operator()(const cv::Range& range) const override {
    ...
    for (int fi = range.start; fi < range.end; fi++) {
      ...
      if (is_buf_16u)
        std::sort(udst + (size_t)fi * sample_count,
                  udst + (size_t)(fi + 1) * sample_count,
                  LessThanIdx<float, unsigned short>(valCachePtr));
      else
        std::sort(idst + (size_t)fi * sample_count,
                  idst + (size_t)(fi + 1) * sample_count,
                  LessThanIdx<float, int>(valCachePtr));
    }
  }
...
struct FeatureValAndIdxPrecalc : cv::ParallelLoopBody {
  FeatureValAndIdxPrecalc(const CvFeatureEvaluator* _featureEvaluator,
                          CvMat* _buf, cv::Mat* _valCache, int _sample_count,
                          bool _is_buf_16u) {
    featureEvaluator = _featureEvaluator;
    valCache = _valCache;
    sample_count = _sample_count;
  ...
  void operator()(const cv::Range& range) const override {
    for (int fi = range.start; fi < range.end; fi++) {
      ...
      if (is_buf_16u)
        std::sort(udst + (size_t)fi * sample_count,
                  udst + (size_t)(fi + 1) * sample_count,
                  LessThanIdx<float, unsigned short>(valCache->ptr<float>(fi)));
      else
        std::sort(idst + (size_t)fi * sample_count,
                  idst + (size_t)(fi + 1) * sample_count,
                  LessThanIdx<float, int>(valCache->ptr<float>(fi)));
    }
  }
  const CvFeatureEvaluator* featureEvaluator;
  cv::Mat* valCache;
...
struct FeatureValOnlyPrecalc : cv::ParallelLoopBody {
  FeatureValOnlyPrecalc(const CvFeatureEvaluator* _featureEvaluator,
                        cv::Mat* _valCache, int _sample_count) {
    featureEvaluator = _featureEvaluator;
    valCache = _valCache;
    sample_count = _sample_count;
  }
...
void CvCascadeBoostTrainData::precalculate() {
  ...
  parallel_for_(cv::Range(0, minNum),
                FeatureValAndIdxPrecalc(featureEvaluator, buf, &valCache,
                                        sample_count, is_buf_16u != 0));
  parallel_for_(
