...
void CvCascadeBoostTree::write(cv::FileStorage& fs, const cv::Mat& featureMap) {
  ...
  while (!internalNodesQueue.empty()) {
    ...
    if (!tempNode->right->left &&
        !tempNode->right->right)  // right node is leaf
    {
    ...
    if (!maxCatCount)
      ...
      for (int i = 0; i < subsetN; i++) fs << tempNode->split->subset[i];
    internalNodesQueue.pop();
  }
  fs << "]";  // CC_INTERNAL_NODES

  fs << CC_LEAF_VALUES << "[:";
  for (int ni = 0; ni < -leafValIdx; ni++) fs << leafVals[ni];
  fs << "]";  // CC_LEAF_VALUES
  fs << "}";
}
  ...
  for (size_t i = 0; i < rnode.size() / step; i++) {
    ...
    if (maxCatCount > 0) {
      ...
      for (int j = subsetN - 1; j >= 0; j--) {
        *internalNodesIt >> prntNode->split->subset[j];
        internalNodesIt++;
      }
    } else {
