                  ...
                  int* var_all, int* sample_all )
{
    ...
    if( tflag != cv::ml::ROW_SAMPLE && tflag != cv::ml::COL_SAMPLE )
        CV_ERROR( cv::Error::StsBadArg,
        "Unknown training data layout (must be cv::ml::ROW_SAMPLE or cv::ml::COL_SAMPLE)" );

    if( var_all )
        *var_all = tflag == cv::ml::ROW_SAMPLE ? train_data->cols : train_data->rows;
                                         ...
                                         int storage_idx, int offset )
{
    ...
    if( params.cv_folds > 0 && cv_heap )
    {
        ...
        node->cv_node_risk = (double*)cv::alignPtr(node->cv_Tn + cv_n, sizeof(double));
